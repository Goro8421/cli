...
GitHub CLI sends usage data to help us understand how the tool is being used and to prioritize improvements. We want to be transparent about what we collect and why.
...
```json
{
  "eventType": "usage",
  "dimensions": {
    "command": "<command path, e.g. gh pr create>",
    "device_id": "<random UUID, generated once per user>",
...
| `device_id` | A random identifier generated once and stored locally. This is **not** a machine ID — different users on the same machine get different identifiers. See [Deleting your device ID](#deleting-your-device-id) below. |
| `platform` | The operating system (`darwin`, `linux`, `windows`). |
...
The full implementation can be found in [`internal/telemetry/`](internal/telemetry/) and [`pkg/cmd/send-telemetry/`](pkg/cmd/send-telemetry/).
